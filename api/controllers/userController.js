...
exports.userCtrl = {
    ...
    getMyCourses: async (req, res) => {
        try {
            let user = await UserModel.findOne({ _id: req.tokenData.userData._id })
            let courses = await courseModel.find({ short_id: user.myCourses }).select("short_id img_url name")
            res.json(courses)
        }
        catch (err) {
            console.log(err)
    ...
    addOrRemoveToWishList: async (req, res) => {
        try {
            ...
            await user.save()
            return res.json({ msg: "wishList updated" })
        }
        catch (err) {
            console.log(err)
        }
    ...
    getWishList: async (req, res) => {
        try {
            let user = await UserModel.findOne({ _id: req.tokenData.userData._id })
            let wishList = await courseModel.find({ short_id: user.wishlist }).select("name short_id img_url")
    ...
    getMyCart: async (req, res) => {
        try {
            let user = await UserModel.findOne({ _id: req.tokenData.userData._id })
            let myCart = await courseModel.find({ short_id: user.myCart }).select("name short_id img_url")
    ...
    getMyInfoAcount: async (req, res) => {
        try {
            let user = await UserModel.findOne({ _id: req.tokenData.userData._id }).select("fullName email phone imgProfile")
            res.json(user)
        }
        catch (err) {
            console.log(err)
